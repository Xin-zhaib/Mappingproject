 ...
 * 判断题数据库的增删改查
 */
public class PanduanDbutils {
    public static final String DB_NAME="panduan_dbname";
    public static final int VERSION=1;
    private static PanduanDbutils sqliteDB;
    private SQLiteDatabase db;

    private PanduanDbutils(Context context){
        PanduanHelper peifangHelper=new PanduanHelper(context,DB_NAME,null,VERSION);
        db=peifangHelper.getWritableDatabase();
    }

    public synchronized static PanduanDbutils getInstance(Context context){
        if(sqliteDB==null){
            sqliteDB=new PanduanDbutils(context);
        }
        return sqliteDB;
    }
    public void delete(Context context, String id){
        ...
        db.delete("Panduan","id=?",new String[]{id});
    }
    ...
    public void change(Context context,Question peifang){
        ...
        db.update("Panduan",values,"id=?",new String[]{peifang.getId()+""});
    }

    public int insert(Question peifang){
        try {
            db.execSQL("insert into Panduan(leixing,wenti,daan)values(?,?,?)",new String[]{
                    peifang.getLeixing(),
                    peifang.getWenti(),
                    peifang.getDaan(),
            });
    ...
    public List<Question>load(){
        ...
        Cursor cursor=db.query("Panduan",null,null,null,null,null,null);
